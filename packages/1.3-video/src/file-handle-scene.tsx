...
const LINES: Line[] = [
  ...
  [],
  [["await", "kw"], [" photo."], ["upload", "fn"], ["(blob)"]],
  [["await", "kw"], [" photo."], ["download", "fn"], ["()"]],
  [
    ["const", "kw"],
    [" link = "],
    ["await", "kw"],
  ...
  [["await", "kw"], [" photo."], ["delete", "fn"], ["()"]],
];

const TOTAL = totalChars(LINES);
const TYPE_FRAMES = Math.ceil((TOTAL / CHARS_PER_SEC) * FPS);
const DWELL = 40;
export const FILE_HANDLE_SCENE_DURATION = ENTER + TYPE_FRAMES + DWELL + EXIT;

export const FileHandleScene: React.FC = () => {
  ...
  const exitOpacity = interpolate(
    frame,
    [FILE_HANDLE_SCENE_DURATION - EXIT, FILE_HANDLE_SCENE_DURATION - 2],
    [1, 0],
