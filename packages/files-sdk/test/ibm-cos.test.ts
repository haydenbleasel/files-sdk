...
import { ibmCos } from "../src/ibm-cos/index.js";
import { Files } from "../src/index.js";

describe("ibm-cos adapter", () => {
  test("derives endpoint from region and uses virtual-hosted style by default", async () => {
    const adapter = ibmCos({
      accessKeyId: "AKID",
      bucket: "uploads",
      region: "us-south",
      secretAccessKey: "SECRET",
    });
    expect(adapter.name).toBe("ibm-cos");
    const client = adapter.raw as S3Client;
    expect(await client.config.region()).toBe("us-south");
  ...
  test("region override flows to both the inner S3 client and the derived endpoint", async () => {
    ...
    const client = adapter.raw as S3Client;
    expect(await client.config.region()).toBe("eu-de");
    const endpoint = await client.config.endpoint?.();
    expect(endpoint?.hostname).toBe(
  ...
  test("missing credentials throws at construction", () => {
    const oldKey = process.env.IBM_COS_ACCESS_KEY_ID;
    const oldSecret = process.env.IBM_COS_SECRET_ACCESS_KEY;
    delete process.env.IBM_COS_ACCESS_KEY_ID;
    delete process.env.IBM_COS_SECRET_ACCESS_KEY;
    try {
      expect(() => ibmCos({ bucket: "uploads", region: "us-south" })).toThrow(
        /credentials/u
      );
    } finally {
      if (oldKey) {
        process.env.IBM_COS_ACCESS_KEY_ID = oldKey;
      }
      if (oldSecret) {
        process.env.IBM_COS_SECRET_ACCESS_KEY = oldSecret;
      }
    }
  });

  test("picks up credentials from IBM_COS_ACCESS_KEY_ID / IBM_COS_SECRET_ACCESS_KEY env vars", async () => {
    const oldKey = process.env.IBM_COS_ACCESS_KEY_ID;
    const oldSecret = process.env.IBM_COS_SECRET_ACCESS_KEY;
    process.env.IBM_COS_ACCESS_KEY_ID = "ENV_KEY";
    process.env.IBM_COS_SECRET_ACCESS_KEY = "ENV_SECRET";
    try {
      const adapter = ibmCos({
        bucket: "uploads",
        region: "us-south",
  ...
  test("url() returns a presigned GET URL by default", async () => {
    ...
    expect(url).toContain("s3.us-south.cloud-object-storage.appdomain.cloud");
  });
  ...
  test("default error messages from the inner s3 adapter are relabeled as 'IBM Cloud Object Storage error'", async () => {
    const { mapS3Error } = await import("../src/s3/index.js");
    const ibmMessages = {
      Conflict: "Conflict",
      NotFound: "Not found",
      Provider: "IBM Cloud Object Storage error",
      Unauthorized: "Unauthorized",
    } as const;
    const err = mapS3Error({ $metadata: { httpStatusCode: 500 } }, ibmMessages);
    expect(err.code).toBe("Provider");
    expect(err.message).toBe("IBM Cloud Object Storage error");
  });
});
