...
An aborted call rejects with a `Provider` [`FilesError`](/docs/api/errors) whose `aborted` flag is `true`. That flag, not the `code`, is what distinguishes a cancellation (or a [timeout](/docs/timeouts)) from a real provider failure.
...
Aborts are never retried — see [Retries](/docs/retries) — and a [`timeout`](/docs/timeouts) aborts the call the same way when it fires.
