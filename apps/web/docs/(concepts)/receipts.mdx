---
title: Receipts
description: Opt into a provenance Receipt for every mutating call - op, provider, key, bytes, etag, timing, optional SHA-256 - delivered on the onAction hook.
---
...
Receipts ride on the existing [`onAction`](/docs/api/onaction) hook as an additive `receipt` field - there's no new method, callback, or changed return type. The field is present **only** when:
