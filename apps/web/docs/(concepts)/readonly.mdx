...
Every write surface throws [`FilesError`](/docs/api/errors) with `code: "ReadOnly"`:
...
Read-only applies to the unified `Files` API only. The [`raw`](/docs/escape-hatch) escape hatch still exposes the underlying provider client unchanged, so code that writes through `files.raw` bypasses the guard by design.
