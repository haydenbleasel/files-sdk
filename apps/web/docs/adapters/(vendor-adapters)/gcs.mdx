...
description: Google Cloud Storage via the official @google-cloud/storage SDK. Application Default Credentials by default, resumable uploads when onProgress is passed.
---
...
`upload` reports true byte-level [progress](/docs/api/upload#progress-tracking) via `onProgress`. Uploads default to a single simple request; when `onProgress` is passed, the adapter switches to a **resumable** upload (the only path that emits progress), which adds one round trip.
