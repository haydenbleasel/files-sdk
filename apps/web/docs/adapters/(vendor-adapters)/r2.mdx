...
Inside Cloudflare Workers (detected via `navigator.userAgent === "Cloudflare-Workers"`), the fetch client is the default: the `"aws-sdk"` client's XML parsing needs `DOMParser`, which workerd doesn't provide, so it fails at runtime on the first list or error-body parse. Pass `client: "aws-sdk"` explicitly to override.
