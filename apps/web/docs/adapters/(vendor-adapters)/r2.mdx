...
Inside Cloudflare Workers (detected via `navigator.userAgent === "Cloudflare-Workers"`, falling back to the workerd-only `WebSocketPair` global when `navigator` is disabled), the fetch client is the default: the `"aws-sdk"` client's XML parsing needs `DOMParser`, which workerd doesn't provide, so it fails at runtime on the first list or error-body parse. Pass `client: "aws-sdk"` explicitly to override.

### Generic S3-compatible endpoints

The engine behind `client: "fetch"` is exported as `s3FetchAdapter` from `files-sdk/r2` for talking to any S3-compatible endpoint (AWS S3, MinIO, Tigris, ...) on runtimes where `files-sdk/s3` can't run — that entry statically imports `@aws-sdk/client-s3`, which has the same `DOMParser` problem on workerd. It takes an explicit `endpoint`, `region`, `bucket`, credentials, and an optional `forcePathStyle`, with the same coverage and trade-offs as the fetch client above.

```ts lineNumbers
import { s3FetchAdapter } from "files-sdk/r2";
...
R2 does not advertise the SDK's provider-native [conditional operation](/docs/conditional-operations) contract in this release. That includes the AWS-SDK HTTP path: configuring an S3-compatible endpoint does not inherit AWS S3's capability claims. Conditional calls fail before R2 I/O.
