...
The separate body and sidecar layout cannot provide a cross-process atomic ETag compare-and-set boundary. The adapter therefore advertises no [conditional operations](/docs/conditional-operations) and rejects them before filesystem I/O; it does not emulate CAS with a read followed by a rename.
