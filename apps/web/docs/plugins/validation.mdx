---
title: validation
description: A fail-closed guard that vets every upload before any bytes move - enforce a max/min size, an allowed-MIME-type list, and a key-naming rule.
---

The built-in `validation()` plugin is a **fail-closed guard**: it checks each write against the rules you set and rejects a bad one by throwing, so no bytes ever reach the adapter. It's the simplest kind of [wrap plugin](/docs/plugins#wrap-intercepting-operations) - it vetoes rather than transforms.

Unlike [`compression()`](/docs/plugins/compression) and [`encryption()`](/docs/plugins/encryption), it never touches the body or writes any metadata, so there's nothing to undo on the way back out. It has **no native dependencies** and works on any adapter.
...
`maxSize` and `minSize` are byte counts. The check needs the body's length, so for an unknown-length stream the plugin buffers the body to measure it (the same trade-off the [buffering plugins](/docs/plugins/compression) make). Known-length bodies - strings, `Uint8Array`, `Blob`, `ArrayBuffer` - are measured without a copy.
...
- **Size rules buffer unknown-length streams.** Measuring a stream means draining it, which is incompatible with [resumable uploads](/docs/resumable). Key and type rules never touch the body, so a key/type-only policy stays fully streaming.
- **It's a guard, not a sanitizer.** It vets the _declared_ type and the key; it doesn't sniff magic bytes or rewrite anything. Pair it with [`contentType()`](/docs/plugins/content-type) if you don't trust the client-declared type.
