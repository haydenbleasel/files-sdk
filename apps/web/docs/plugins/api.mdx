...
An `upload` handler therefore sees overwrite, conditional-create, and conditional-replace modes. Existing body-transforming handlers cover conditional writes automatically. The engine preserves a conditional root's family, mode, and predicate across every `next()` call and requires exactly one native operation before success.
