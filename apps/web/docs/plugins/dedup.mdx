---
title: dedup
description: Content-address object bodies by SHA-256 hash so identical content is stored only once - re-uploads skip the byte upload and copies share one blob.
---
...
- **`download`** follows the pointer to the blob and returns it under your key — **ranges included**, because blobs are stored verbatim (unlike [`compression()`](/docs/plugins/compression)).
...
- **It buffers the whole body** to hash it, so — like [`compression()`](/docs/plugins/compression) — it's unsuitable for unknown-length [streaming uploads](/docs/api/upload) and [resumable uploads](/docs/resumable).
