---
title: failover
description: Read and write the primary adapter and fall back to one or more secondary adapters when a backend is down - a live, per-operation failover chain.
---
...
It's **body-transparent**: it never buffers or transforms bytes, so streaming, [range downloads](/docs/api/download), [`url()`](/docs/api/url), and [`signedUploadUrl()`](/docs/api/signed-upload-url) all keep working. It has **no native dependencies**, adds no methods (`wrap` only, so plain `new Files()` is enough), and works on any set of adapters.
...
| [`tiering()`](/docs/plugins/tiering) | **Partition** objects across backends by key / size / age. |
...
By default, the plugin fails over **only** on a `Provider` [error](/docs/api/errors) - a network failure, timeout, or 5xx, i.e. "the backend is down" - and **never** on an aborted request. A definitive answer from a _healthy_ backend is surfaced as-is, not masked by probing a replica:
...
Pass `shouldFailover` to change the rule. It receives the error normalized to a [`FilesError`](/docs/api/errors) (so `code` and `aborted` are always set). For example, to **read through** to a replica on a miss - useful when the secondary is a live mirror that may be ahead of the primary:
...
[Bulk](/docs/plugins#bulk-operations-too) calls fan out to one operation per item, so each element fails over independently.
...
- reconcile after an outage with [`sync`](/docs/api/sync) / [`transfer`](/docs/api/transfer), or
- pass a `shouldFailover` that also fails over on `NotFound` so reads fall through to the replica.

## Ordering and prefixes

- **Place it last (innermost).** Body-transforming plugins like [`encryption()`](/docs/plugins/encryption) and [`compression()`](/docs/plugins/compression) wrap `failover()` and transform the op on the way in, so the same bytes reach **every** backend:
...
- **Instance defaults apply to every backend.** The `timeout`, `retries`, and `signal` you pass to the constructor govern the secondaries as well as the primary (per-call options forward too), so a hung replica is cut off the same way a hung primary is rather than stalling the chain after a failover.
