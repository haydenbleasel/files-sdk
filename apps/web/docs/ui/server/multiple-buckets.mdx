...
:::note
The selector is client-supplied, so treat it as untrusted input: `authorize` must enforce who may touch which bucket, exactly as it does for keys. The upload tokens `presign` mints are bound to the query they were minted under, so a token issued for `?bucket=images` is refused by the proxy upload and `complete` steps if it is presented with a different selector.
:::
