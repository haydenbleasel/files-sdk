...
`files-sdk/hono` mounts a [`createFilesRouter`](/docs/ui/server/gateway) in a Hono app. Like the [Next binding](/docs/ui/server/next) it is Web-native, so it runs anywhere Hono does — **Cloudflare Workers, Bun, Deno, and Node**.
