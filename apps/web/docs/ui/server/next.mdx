...
That one route backs every client binding — the [React hook](/docs/ui/client/react), [Svelte](/docs/ui/client/svelte), and [Vue](/docs/ui/client/vue). See the [gateway options](/docs/ui/server/gateway) for the full configuration, and lock it down with [`authorize`](/docs/ui/server/authorization) before shipping.
