...
Previews one stored file by key (or a resolved `StoredFile`). Images and PDFs prefer a direct [`url()`](/docs/ui/client/react#downloading-vs-linking), falling back to the gateway download proxy when the adapter can't sign; text is fetched and shown inline. Bytes load only when the component mounts.
