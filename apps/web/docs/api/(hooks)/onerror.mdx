...
The payload mirrors [`onAction`](/docs/api/onaction) - `type`, the caller-facing `key` / `keys` (or `from` / `to` for `copy` and `move`), the redacted `condition` label for a [conditional call](/docs/conditional-operations), and `durationMs` - but `error` is always present and typed as a [`FilesError`](/docs/api/errors), not optional.

## Filtering out aborts

A [cancellation](/docs/cancellations) and a [timeout](/docs/timeouts) both reject, so both reach `onError` - usually as noise you don't want paging anyone. They carry `aborted: true`; branch on it before reporting.
