...
Reads an object. Returns a [`StoredFile`](/docs/api/stored-file) by default (Blob-backed). Pass `{ as: "stream" }` to opt into a `ReadableStream` for large objects.
...
Pass a canonical bare strong ETag to read only that object generation. The predicate is sent in the provider's native read request; there is no preceding `head()` and no fallback on adapters without the primitive.

```ts lineNumbers
const exact = await files.download("avatars/abc.png", {
  condition: { etag: current.etag! },
  as: "stream",
});
```

Exact reads can compose with `range` when the adapter supports both primitives. See [Conditional operations](/docs/conditional-operations).
