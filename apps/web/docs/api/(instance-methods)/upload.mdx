...
## Create and replace conditions

The single-key form accepts a provider-native create or exact-replace condition. A create succeeds only when the destination is absent; a replace succeeds only while its supplied bare strong ETag is current. The SDK does not probe first, and unsupported adapters fail before provider I/O.
...
Conditional uploads return a non-optional ETag. They are not available for array, multipart, resumable, or signed-upload paths; the initial S3 implementation also rejects unknown-length streams before I/O. See [Conditional operations](/docs/conditional-operations) for capabilities, plugin behavior, and ambiguous retry outcomes.
...
- Some adapters report **true byte-level progress for every body type** (buffered included) by tapping their SDK's native upload-progress hook: **S3** and the **S3-compatible** adapters, **R2** (HTTP), **Azure Blob**, **Google Cloud Storage**, **Firebase Storage**, **Vercel Blob**, and **FTP**. Notes:
  - The S3 family (incl. R2 over HTTP with the `"aws-sdk"` client) needs the optional [`@aws-sdk/lib-storage`](https://www.npmjs.com/package/@aws-sdk/lib-storage) package installed; it also enables multipart for large files.
...
- **S3 and the S3-compatible adapters** (incl. R2 over HTTP with the `"aws-sdk"` client) run multipart through the optional [`@aws-sdk/lib-storage`](https://www.npmjs.com/package/@aws-sdk/lib-storage) package, falling back to a single `PutObject` when the body fits in one part. Unknown-length streams use multipart **automatically**, even without the flag.
