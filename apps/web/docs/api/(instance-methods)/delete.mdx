...
**One key** removes a single object and resolves to `void`. No-op friendly: a missing key resolves successfully on providers that treat delete as idempotent, and throws [`FilesError`](/docs/api/errors) with `code: "NotFound"` on ones that don't.

```ts lineNumbers
await files.delete("avatars/abc.png");
```

## Delete only the expected generation

The single-key form accepts an exact ETag predicate. The provider removes the object only if that generation is still current; unsupported adapters and stale predicates fail closed.
...
Conditional delete has no array form. See [Conditional operations](/docs/conditional-operations) for capability and retry semantics.
