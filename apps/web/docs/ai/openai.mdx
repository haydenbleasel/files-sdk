...
Setting `strict: true` on a Responses tool reshapes its `parameters` to what OpenAI strict mode accepts: every property is listed in `required`, optional ones become nullable (`execute` treats `null` as "absent"), and free-form maps are dropped, so `uploadFile` has no `metadata` field under strict mode.
