...
Over the HTTP API, `upload` reports true byte-level [progress](/docs/api#progress-tracking) via `onProgress` when the optional `@aws-sdk/lib-storage` package is installed (it's loaded only when `onProgress` is used). The Workers `R2Bucket` binding reports start/finish instead.
