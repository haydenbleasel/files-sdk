...
| `signedUploadUrl` |   ❌   | Throws — Convex `generateUploadUrl()` cannot bind the caller's SDK key, expiry, size bounds, or content type into the issued upload capability. Upload through a Convex action with `files.upload()` instead, then track the returned storage id.                                                                                                                                                                                                                                                                     |
