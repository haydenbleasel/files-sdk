...
Every method normalizes to the same shape, but providers differ in what they offer natively. Where a provider lacks a primitive, the method throws a `FilesError` rather than silently misbehaving (for example, `signedUploadUrl` on Vercel Blob). The [compatibility matrix](/api/compatibility) shows per-adapter support, and `files.raw` is the [typed escape hatch](/features/escape-hatch) for anything outside the unified surface.
