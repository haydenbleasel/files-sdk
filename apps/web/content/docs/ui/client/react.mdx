...
`files-sdk/react` brings the full Files API to the browser as a React hook. `useFiles` returns one method per `Files` verb against the [gateway](/ui/gateway) you mounted, plus ambient upload/error state. Keys are plain strings (relative to your [authorized prefix](/ui/authorization)).
