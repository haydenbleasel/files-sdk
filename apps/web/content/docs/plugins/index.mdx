...
<Callout>
  You don't have to write these yourself - production-ready
  [`validation()`](/plugins/validation),
  [`compression()`](/plugins/compression), and
  [`encryption()`](/plugins/encryption) plugins ship in the box. They form a
  natural pipeline, run in order: vet the upload, compress what's left, then
  encrypt it - `plugins: [validation({maxSize}), compression(),
  encryption(key)]`.
</Callout>
