...
`maxSize` flips `signedUploadUrl()` from PUT to POST-with-policy to enforce the cap at the bucket via `content-length-range`. Some adapters can't honor that contract:

- **Azure**, **Supabase**, **Cloudinary**, and **UploadThing** throw on `maxSize` - none exposes a URL-level `content-length-range` equivalent.
- **Google Drive**, **OneDrive**, and **SharePoint** throw on `maxSize`/`minSize` for the same reason.
