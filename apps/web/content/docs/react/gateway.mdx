---
title: Gateway
description: createFilesRouter exposes the whole Files API over one HTTP endpoint. Mount it in Next (or any Web-Request runtime) and the browser hook talks to it.
---

`createFilesRouter` turns a `Files` instance into an HTTP handler that the [`useFiles`](/react/use-files) hook calls. It is framework-agnostic — `handle(req: Request): Promise<Response>` — and surfaced by thin bindings like `files-sdk/next`.
...
createFilesRouter({
  ...
  // Per-operation gate. Deny-by-default when omitted. See /react/authorization.
...
Force one with `downloadMode: "redirect" | "proxy"`. Proxied downloads send `Content-Disposition: attachment` by default (so a stored `.html`/SVG can't execute inline at your origin) unless [`authorize`](/react/authorization) returns `{ disposition: "inline" }`.
...
The gateway core accepts any `{ handle(req: Request): Promise<Response> }` consumer, so a binding is a few lines. `files-sdk/next` ships today; Hono and an Express (`Readable.toWeb`/`fromWeb`) bridge are on the roadmap. Until then you can call `router.handle(request)` directly from any handler that gives you a Web `Request`.
