...
The gateway core accepts any `{ handle(req: Request): Promise<Response> }` consumer, so a binding is a few lines. Ready-made adapters ship for **Next.js**, **Hono**, and **Express** — see [Framework adapters](/react/frameworks). For anything else, call `router.handle(request)` directly from any handler that gives you a Web `Request`.
