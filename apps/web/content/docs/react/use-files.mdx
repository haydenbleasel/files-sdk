...
`useFiles` returns one method per `Files` verb, against the [gateway](/react/gateway) you mounted. Keys are plain strings (relative to your [authorized prefix](/react/authorization)).
